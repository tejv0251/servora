...
export async function POST(request: Request) {
  try {
    ...
    const job = await db
      .prepare(
        'SELECT id, service, status FROM jobs WHERE id = ? AND workspace_id = ? LIMIT 1',
      )
      .bind(jobId, workspace.id)
      .first<{ id: string; service: string; status: JobStatus }>();
    if (!job)
      return json(
    ...
    if (!canTransitionJob(job.status, status))
      return json(
        { error: `A job cannot move from ${job.status} to ${status}.` },
        { status: 409 },
      );

    const updated = await db
      .prepare(
        'UPDATE jobs SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND workspace_id = ? AND status = ?',
      )
      .bind(status, job.id, workspace.id, job.status)
      .run();
    if (updated.meta.changes !== 1)
      return json(
        { error: 'The job changed before this update. Refresh and try again.' },
        { status: 409 },
      );
    await db
      .prepare(
